...
{if(!window.event)window.event=e;if(typeof(window.onmousedown_rgraph)=='function'){window.onmousedown_rgraph(e);}
...
{if(!window.event)window.event=e;if(RGraph.Annotating_window_onmouseup){RGraph.Annotating_window_onmouseup(e);return;}
...
{if(!window.event)window.event=e;if(typeof(e.target.onmouseup_rgraph)=='function'){e.target.onmouseup_rgraph(e);}
...
RGraph.Clear(obj.canvas);RGraph.Redraw();obj.Highlight(shape);RGraph.Registry.Set('chart.tooltip.shape',shape)
RGraph.Tooltip(obj,text,x,y,shape['index']);if(RGraph.Registry.Get('chart.tooltip')){RGraph.Registry.Get('chart.tooltip').__shape__=shape;RGraph.EvaluateCursor(e);}
...
{if(!window.event)window.event=e;if(typeof(e.target.onmousemove_rgraph)=='function'){e.target.onmousemove_rgraph(e);}
...
{if(!window.event)window.event=e;if(typeof(e.target.onmousedown_rgraph)=='function'){e.target.onmousedown_rgraph(e);}
...
{if(!window.event)window.event=e;if(typeof(e.target.onclick_rgraph)=='function'){e.target.onclick_rgraph(e);}
...
var id=obj.id;if(!RGraph.is_null(obj)){if(obj.getShape&&obj.getShape(e)){var shape=obj.getShape(e);if(obj.Get('chart.tooltips')){var text=RGraph.parseTooltipText(obj.Get('chart.tooltips'),shape['index']);if(text){var pointer=true;}}}
